...
const MenuForm = ({
  ...
  const validateName = (rule, value, callback) => {
    ...
    axios.get('/sys/menus/menuNameValidation', { params: { q: value } }).then(function (data) {
      callback();
    }, function (error) {
  ...
  const changeMenu = (checked) => {
    if (checked) {
      setFieldsValue({ mpid: currItem.mpid })
    } else {
      setFieldsValue({ mpid: 'submenu' })
    }
  }
    ...
    <div>
      <Form layout="horizontal">
        ...
        <FormItem label="导航菜单" {...formItemLayout}>
          <Switch defaultChecked onChange={changeMenu} checkedChildren={<Icon type="check" />} unCheckedChildren={<Icon type="cross" />} />
        </FormItem>
        ...
        <FormItem label="图标" {...formItemLayout}>
          ...
          })(<Input placeholder="点击图标查找..."
                    suffix={<span style={{ cursor: 'pointer' }} onClick={toLocation}><Icon type="picture" /></span>} />)}
        </FormItem>
